...
// print "You are too tired to move." If there is no room in the specified
// direction, it will print "Alas, you cannot go that way." Move also notifies
// all Mobs remaining in the room of the passage of the Mob that moved and
// notifies all Mobs in the new room of the Mob's arrival.
func (m *Mob) Move(d Direction) string {

	for _, exit := range m.Room.Exits {
		if exit.Direction == d {
			if m.CurrentAttr.Vitals.Mv >= exit.Room.MovementCost {
				m.CurrentAttr.Vitals.Mv -= exit.Room.MovementCost
...
// LeftRoom notifies a Mob that another Mob left a room and in what direction
// they were going.
...
// EnteredRoom notifies a Mob that another Mob entered the same room from a
// given direction.
...
// DecrementDelay decreases the delay before a Mob can move again by 1 if it is
// above zero.
...
// Tick decrements the delay before a Mob can act again and partially
// regenerates depleted attributes (like vitals).
func (m *Mob) Tick() {
	m.DecrementDelay()
	m.Regen()
	//set flag to false
...
// Pulse causes a Mob to act, either by attacking their current target or
// potentially wandering if they are not currently targeting any other Mobs.
func (m *Mob) Pulse() {
...
// Attack causes the Mob to attack whatever its current target is. This
// decrements the target's current Hp Attribute by 5 and notifies both Mobs of
// the attack. If the target's Hp is reduced to 0 or lower, it dies. If the
// attacked Mob survives, but does not have a target, it is set to target the
// attacking Mob.
func (m *Mob) Attack() {
...
// Die informs a Mob of its death and creates a container to represent their
// corpse. This container is automatically added to the contents of a room.
func (m *Mob) Die() {
	m.Notify("You died!\n")
	m.target = nil
	corpse := Item{}
...
// Wander makes a Mob wander one room in a given direction. The percent chance
// of this behavior occurring is based on the Mob.Wanders value.
func (m *Mob) Wander() {
	if rand.Float64() < m.Wanders {
		d := m.Room.Exits
		l := len(d)
		i := rand.Intn(l)
		m.Move(d[i].Direction)
		m.hasWandered = true
	}
}
